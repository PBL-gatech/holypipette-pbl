...
class ScientificaSerialNoEncoder(Manipulator):
    ...
    def set_max_speed(self, speed):
        '''Sets the max speed for the Scientifica Stage.  
           It seems like the range for this is around (1000, 100000)
        '''
        resp = self._sendCmd(SerialCommands.SET_MAX_SPEED.format(int(speed)))
        print(f"speed set set: {resp},{speed}")

    def set_max_accel(self, accel):
        '''Sets the max acceleration for the Scientifica Stage.
           It seems like the range for this is around (10, 10000)
        '''
        resp = self._sendCmd(SerialCommands.SET_MAX_ACCEL.format(int(accel)))
        print(f"acceleration set: {resp},{accel}")
    ...
    def update_pos_continuous(self, freq=40):
        '''constantly polls the device's position and updates the current_pos variable
        '''
        while True:
            startTime = time.time()
            # isBusy = self._sendCmd(SerialCommands.GET_IS_BUSY)
            isBusy = 'text'
            if 's' not in isBusy:
                xyz = self._sendCmd(SerialCommands.GET_X_Y_Z)
                xyz = xyz.split('\t')
    ...
    def absolute_move(self, pos, axis, speed=None):
        ...
        '''
        try: 
            print(f"command pos:{pos}")
            if axis == 1:
                yPos = self.position(axis=2)
                resp = self._sendCmd(SerialCommands.SET_X_Y_POS_ABS.format(int(pos/self.correction) , int(yPos/self.correction)))
                print(f"abs move response: {resp}")
            if axis == 2:
                xPos = self.position(axis=1)
                resp = self._sendCmd(SerialCommands.SET_X_Y_POS_ABS.format(int(xPos/self.correction), int(pos/self.correction)))
                print(f"abs move response: {resp}")
            if axis == 3:
                resp = self._sendCmd(SerialCommands.SET_Z_POS.format(int(pos/self.correction)))
                print(f"abs move response: {resp}")
        except Exception as e:
            self.error(f"Error in absolute_move: {e}")
    ...
    def absolute_move_group(self, x, axes, speed=None):
        ...
        x = list(x)
        axes = list(axes)
        print(f"command pos group:{x}")
        if 1 in axes and 2 in axes and 3 not in axes:
            # Move X and Y axes together
            xPos = x[axes.index(1)]
            yPos = x[axes.index(2)]
            resp = self._sendCmd(SerialCommands.SET_X_Y_POS_ABS.format(int(xPos/self.correction ), int(yPos/self.correction )))
            print(f"abs move group response: {resp}")
        elif 1 in axes and 2 in axes and 3 in axes:
            # Move X, Y and Z axes together
            xPos = x[axes.index(1)]
            yPos = x[axes.index(2)]
            zPos = x[axes.index(3)]
            resp = self._sendCmd(SerialCommands.SET_X_Y_Z_POS_ABS.format(int(xPos/self.correction ), int(yPos/self.correction ), int(zPos/self.correction )))
            print(f"abs move group response: {resp}")
    ...
    def absolute_move_group_velocity(self, vel):
        ...
        try: 
            ...
            xvel, yvel, zvel = vel
            resp = self._sendCmd(SerialCommands.SET_X_Y_Z_VEL.format(xvel, yvel, zvel))
            print(f"abs move velocity response: {resp}")
        except Exception as e:
            self.error(f"Error in absolute_move: {e}")
    ...
    def relative_move_group(self, pos, axis, speed=None):
        ...
        if axis == 1:
            resp = self._sendCmd(SerialCommands.SET_X_Y_POS_REL.format(pos/self.correction, 0))
            print(f"rel move group response: {resp}")
        if axis == 2:
            self._sendCmd(SerialCommands.SET_X_Y_POS_REL.format(0, pos/self.correction))
            print(f"rel move group response: {resp}")
        if axis == 3:
            absZCmd = self.position(3) + pos
            self.absolute_move(absZCmd, 3)
    ...
    def relative_move_group(self, x, axes, speed=None):
        ...
        if cmd[2] != 0:
            ...
            print(f"rel move velocity response: {resp}")
    ...
    def stop(self):
        resp = self._sendCmd(SerialCommands.STOP)
        print(f"stop command sent: {resp}")
