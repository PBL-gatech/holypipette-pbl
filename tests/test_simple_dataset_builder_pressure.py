...
def test_pressure_delta_action_targets_commands_after_observation():
    builder = _pressure_builder(
        ActionSelector(
            include_stage=False,
            include_pipette=False,
            include_pressure=True,
            pressure_use_raw_values=False,
        )
    )
    graph_values = np.asarray(
        [
    ...
    delta_pressure, target_atm_state = builder.get_attempt_pressure_action_values(
        graph_values,
        _pressure_events(),
    )

    np.testing.assert_array_equal(delta_pressure, np.asarray([-5.0, 0.0, 0.0]))
    np.testing.assert_array_equal(target_atm_state, np.asarray([0.0, 1.0, 1.0]))


def test_binary_pressure_actions_compare_next_target_to_current_state():
    builder = _pressure_builder(
        ActionSelector(
            include_stage=False,
            include_pipette=False,
            include_pressure=True,
            pressure_use_binary_actions=True,
        )
    )
...
def test_gigaseal_since_last_action_observations_ignore_current_action_target():
    actions = np.asarray([[-5.0], [0.0], [5.0], [0.0]], dtype=np.float64)
    labels = ["delta_pressure_mbar"]
    ...
    np.testing.assert_array_equal(counts, np.asarray([0.0, 0.0, 1.0, 0.0]))
    np.testing.assert_array_equal(elapsed, np.asarray([0.0, 0.0, 1.0, 0.0]))
...
def test_gigaseal_pressure_observation_keeps_continuous_graph_pressure():
    ...
    builder.observation_selector = ObservationSelector(
        ...
        include_gigaseal_log_resistance=False,
