...
class AdaptiveSlidingModeController:
    ...
    def __init__(self, target_resistance_mohm, parameters=None, parameter_path=None):
        ...
        self.pipette_radius_m = float(model["pipette_radius_um"]) * 1e-6
        self.liquid_layer_thickness_m = float(model["liquid_layer_thickness_nm"]) * 1e-9
        self.seal_media_resistivity_ohm_m = float(model["seal_media_resistivity_ohm_m"])
        self._resistance_to_length_m = (
            2.0 * math.pi * self.pipette_radius_m * self.liquid_layer_thickness_m
    ...
    def _derive_nominal_model_parameters(self, model):
        ...
        mass_kg = float(model["membrane_mass_g"]) * 1e-3
        ...
        membrane_potential_v = float(model["membrane_surface_potential_mv"]) * 1e-3
        pipette_potential_v = float(model["pipette_surface_potential_mv"]) * 1e-3
        potential_gradient_v_per_m = float(model["surface_potential_gradient_mv_per_nm"]) * 1e6
        self.model_a = -viscous_kg_per_s / mass_kg
        self.model_b = -(adhesion_n_per_m2 * 2.0 * math.pi * self.pipette_radius_m + elastic_n_per_m) / mass_kg
        self.model_c = math.pi * self.pipette_radius_m ** 2 / mass_kg
        self.model_d = (
            2.0 * math.pi * self.pipette_radius_m * epsilon_0 * epsilon_r / mass_kg
            * ((membrane_potential_v - pipette_potential_v) / self.liquid_layer_thickness_m
    ...
    def resistance_to_length(self, resistance_mohm):
        ...
        """
        return float(resistance_mohm) * 1e6 * self._resistance_to_length_m
